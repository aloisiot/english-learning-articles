...
| ~~2~~ | ~~**Whether the `/class` rewrite works at all in production.**~~ **Closed 2026-09-02 — and the answer was no.** A `vercel.json`/`vercel.ts` rewrite to another project never applied: `headers` from the same config reached the edge while `rewrites` did not, across four deploys. `/class` is now routed by a Vercel **microfrontends** group (`site/microfrontends.json`), which worked immediately. `basePath: "/class"` composes with it correctly — `/class`, `/class/j/<token>` and `/class/admin/<key>` all serve, and assets load. **Microfrontends is available on the Hobby plan**, which had been a separate worry. | Resolved by deploying, which is what the item asked for. The same-domain arrangement in `07` §4 stands; only the mechanism changed. | `07` §4 and `08` Phase 4 |
| ~~2b~~ | ~~**Whether Deployment Protection on the class project would break the rewrite.**~~ **Closed 2026-09-02.** It never obstructed anything: `/class` serves through the microfrontends group with protection as configured. | Resolved incidentally by the same deploy. | `08` Phase 4 |
...
| ~~4~~ | ~~**Whether npm workspaces installs work cleanly with per-project Vercel Root Directories.**~~ **Closed 2026-09-02.** Both projects build with `installCommand: "cd .. && npm ci"` and *Include files outside the Root Directory*; the build log shows the root install running before config compilation and resolving 258 packages. | Resolved by deploying. | `07` §5 |
...
| 10 | **Whether the site can publish `/articles/<slug>.json` under `output: "export"` as cleanly as the dictionary shards do.** | Reasoned by analogy from `dictionary/[shard]/route.js`, which is verified to work — and now verified *in production*, where `/dictionary/<shard>` serves 200 through the deployed static export. The analogy is stronger than it was; it still has not been built. | `07` §3 — the content seam the whole split rests on. Now `08` Phase 6, having moved behind accounts |
