...
| 2 | **Whether the `/class` rewrite works at all in production.** Grew on 2026-08-30 into three stacked unknowns: that a rewrite can target another Vercel project's production URL; that `basePath: "/class"` composes with it for assets; and that Vercel evaluates `site/vercel.ts` at build time as expected, since the config is no longer static JSON. | Nothing has been deployed — the Vercel account has zero projects. One deploy must prove all three, and a failure entangles them. | `07` §4 and `08` Phase 4 — the entire same-domain arrangement, and the claim that the site keeps the domain and the blast radius stays small |
| 2b | **Whether Deployment Protection on the class project would break the rewrite.** | Identified 2026-08-30, not tested. If protection covers the production deployment, the site's rewrite proxies into an auth wall — and the symptom looks like a routing bug, so it can absorb a debugging session that belongs to item 2. | `08` Phase 4 |
