...
(the rule lives in `docs/article-format.md`). Two consequences for this document:
...
Optional per article (`cover_image`, `cover_image_thumb`,
`cover_image_alt`, `cover_image_credit` front matter — see the
`english-learning-cover-image` skill for sourcing). When present:
...
**File weight.** `cover_image` is a WebP file, pre-compressed by
`download-covers.mjs` (resized to max 1600px wide, quality 80) —
never the original Wikimedia Commons file, which can run several
megabytes at full resolution. `cover_image_thumb` is a second, smaller
WebP (max 500px, quality 75) generated by the same script for use in
article-list rows (§6b), not this hero placement — the two exist
because a hero image and a 144px-wide list thumbnail have very
different size needs, and serving the hero file at thumbnail size
would mean downloading far more data than a list row actually
displays. `loading="eager"`/`decoding="async"` on this image (it's
always above the fold, unlike the list thumbnails).
...
- `loading="lazy"`/`decoding="async"` — unlike the article page's hero
  image, most of these rows sit below the fold, and there can be many
  of them on one page (the full homepage list, or a page of search
  results), so deferring off-screen ones is a real save.
- Renders `article.cover_image_thumb` (the dedicated small file — see
  §6a's "File weight"), falling back to `cover_image` only for content
  that predates the thumbnail field. Never the hero file at full size
  shrunk by CSS — that was the original implementation and meant every
  list row downloaded a hero-sized image (in one case several MB) just
  to show a 144px crop of it.
...
syntax directly on the `<img>` element, and `cover_image_thumb`
separately (it isn't rendered as an `<img>` on the article page itself,
only the hero is, so it needs the literal `key:value` form instead of
`key[attr]`):
...
{article.cover_image_thumb && (
  <div hidden data-pagefind-meta={`cover_image_thumb:${article.cover_image_thumb}`} />
)}
```
...
that series in `site/content/series.json` — see `docs/series.md`)
the throughline sentence(s) as a `.series-description`
